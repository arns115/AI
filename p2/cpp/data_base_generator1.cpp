...
int calc_corners_index(const vector<int> &cur){
  ...
  vector<int> corner_permutation(8);
  vector<int> where(54);
  for(int i = 0; i < 54; i++){
    where[cur[i]] = i;
  }
  ...
  for(int i = 0; i < 7; i++){
    orientation_corners[i] = calc_corner_orientation(cur[corner_triplets[i][0]] / 9, cur[corner_triplets[i][1]] / 9, cur[corner_triplets[i][2]] / 9, corner_triplets[i][0] / 9);
  }
  // get the corresponding permutation
  for(int i = 0; i < 8; i++){
    corner_permutation[i] = calc_corner_index(where[corner_triplets[i][0]] / 9, where[corner_triplets[i][1]] / 9, where[corner_triplets[i][2]] / 9);
    LL[corner_permutation[i]]++;
  }
...
int main(){
  ...
  for(auto [x, y, z]: corner_triplets){
    B.push_back(calc_corner_index(scrambled[x] / 9, scrambled[y] / 9, scrambled[z] / 9));
  }
  for(int x: initial_state){
    cout << x << " ";
  }
  cout << endl;
  sort(A.begin(), A.end());
